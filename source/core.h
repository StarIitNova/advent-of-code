...
namespace AOC {
    ...
    template <typename T>
    bool in(const std::vector<T> &vec, T v) {
    ...
    template <typename T>
    std::vector<T> intersect(std::vector<T> vec, std::vector<T> bAnd) {
    ...
    template <typename T>
    std::vector<T> intersectOr(std::vector<T> vec, std::vector<T> bOr) {
        std::sort(vec.begin(), vec.end());
    ...
    template <typename T>
    std::vector<T> subvec(std::vector<T> vec, size_t start, size_t end = -1, int step = 1) {
        std::vector<T> gen{};
    ...
    template <typename T>
    bool vecEquals(std::vector<T> a, std::vector<T> b) {
        if (a.size() != b.size())
            return false;
